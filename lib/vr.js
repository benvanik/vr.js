...
vr.DriverDataSource.prototype.queryHmdInfo = function() {
  var rawDesc = this.driver_.hmdDeviceDesc;
  var rawInfo = this.driver_.hmdInfo;
  if (!rawDesc || !rawInfo) {
    return null;
  }
  var info = new vr.HmdInfo();
  info.deviceName = rawDesc.productName;
  info.deviceManufacturer = rawDesc.manufacturerName;
  info.deviceVersion = 0;
  info.resolutionHorz = rawInfo.resolutionHorz;
  info.resolutionVert = rawInfo.resolutionVert;
  info.screenSizeHorz = rawInfo.screenSizeHorz;
  info.screenSizeVert = rawInfo.screenSizeVert;
  info.screenCenterVert = rawInfo.screenCenterVert;
  info.eyeToScreenDistance = rawInfo.eyeToScreenDistance[0];
  info.lensSeparationDistance = rawInfo.lensSeparation;
  info.interpupillaryDistance = 0.064;
  info.distortionK = new Float32Array([
    1.0, 0.22, 0.24, 0.0
  ]);
  info.chromaAbCorrection = new Float32Array([
...
vr.DriverDataSource.prototype.poll = function(state) {
  state.hmd.present = !!this.driver_.hmdInfo;
  state.hmd.rotation[0] = this.driver_.rotation[0];
  state.hmd.rotation[1] = this.driver_.rotation[1];
  state.hmd.rotation[2] = this.driver_.rotation[2];
  state.hmd.rotation[3] = this.driver_.rotation[3];
};
