...
// The API will be made a bit cleaner in the future. For example, it will likely
// have events for device addition/removal, as well as settings as to whether
// to handle those devices.
// In instances where things can be made easier via native APIs those may be
// exposed to, guarded behind a permissions popup. Think things like moving the
// current tab to the Oculus screen.
...
vr.beginFullScreen = function() {
  var oculusInfo = vr.queryOculusInfo() || new vr.VROculusInfo();
  if (!oculusInfo) {
    return false;
  }
